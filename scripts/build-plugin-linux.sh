...
get_search_roots() {
  ...
  for root in "/usr/local" "/opt" "$HOME" "$HOME/apps" "$HOME/Applications"; do
    if [[ -z "${seen[$root]+x}" ]]; then
      roots+=("$root")
      seen[$root]=1
    fi
...
parse_nuke_install_name() {
  local name="$1"
  if [[ "$name" =~ ^Nuke([0-9]+\.[0-9]+)v([0-9]+)$ ]]; then
...
get_installed_nukes() {
  ...
  local -A best_patch=()
  local -A best_record=()
  local root dir name parsed minor patch full source
  ...
  while IFS= read -r root; do
    ...
    if [[ -n "$parsed" ]]; then
      found_by_dir["$root"]="${parsed}|${root}|filesystem"
    fi
    ...
    while IFS= read -r -d '' dir; do
      ...
      parsed="$(parse_nuke_install_name "$name" || true)"
      if [[ -n "$parsed" ]]; then
        found_by_dir["$dir"]="${parsed}|${dir}|filesystem"
      fi
...
copy_build_artifact() {
  ...
  for candidate in \
    "${build_dir}/${configuration}/gifWriter.so" \
    "${build_dir}/gifWriter.so"; do
    if [[ -f "$candidate" ]]; then
      artifact="$candidate"
      break
    fi
  ...
  cp -f "$artifact" "${artifacts_dir}/${minor_version}/gifWriter.so"
}

usage() {
  cat <<'EOF'
Usage: build-plugin-linux.sh [options]
